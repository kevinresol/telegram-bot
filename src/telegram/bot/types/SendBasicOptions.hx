...
typedef SendBasicOptions = {
	final ?disable_notification : Bool;
	final ?reply_to_message_id : Int;
	final ?reply_markup : ReplyMarkup;
};

@:jsonStringify(function(v:telegram.bot.types.SendBasicOptions.ReplyMarkup):telegram.bot.types.SendBasicOptions.ReplyMarkupEither return (switch v {
	case InlineKeyboardMarkup(v): v;
	case ReplyKeyboardMarkup(v): v;
	case ReplyKeyboardRemove(v): v;
	case ForceReply(v): v;
}:telegram.bot.types.SendBasicOptions.ReplyMarkupEitherAbstract))
enum ReplyMarkup {
